...
def calculate_salary_report(emp_id, base_salary, joining_date_str):
    ...
    return full_present_days, half_days, actual_absents, late_days, net_payable, total_deduction
...
if st.session_state.logged_in_uid != "":
    ...
    # --- EMPLOYEE VIEW ---
    if user_info["Role"] == "Employee":
        f_days, h_days, a_days, l_days, p_sal, ded = calculate_salary_report(current_uid, user_info['Base_Salary'], user_info['Joining_Date'])
        ...
        with c3: st.metric(label="❌ Effective Absents", value=f"{a_days} Days")
        with c4: st.metric(label="📉 Total Salary Deducted", value=f"₹{ded}")
            
        st.info(f"আপনার লেট হাজিরার সংখ্যা: {l_days} দিন। প্রতিদিনের অনুপস্থিতি এবং হাফ-ডের মোট হিসাব করে স্যালারি ডিডাকশন (Deduction) লাইভ দেখানো হচ্ছে।")
    ...
    # --- OWNER / ADMIN PANEL ---
        ...
        for uid, udata in all_users.items():
            if udata["Role"] == "Employee":
                f_days, h_days, a_days, l_days, p_sal, ded = calculate_salary_report(uid, udata['Base_Salary'], udata.get('Joining_Date', '2026-01-01'))
                ...
                report_rows.append({
                    ...
                    "Full": f_days, "Half": h_days, "Late": l_days, "Payable": p_sal, "Deducted": ded, "Absent_Days": a_days
                })
        ...
        if not df_to_show.empty:
            for _, row in df_to_show.iterrows():
                ...
                cs1, cs2, cs3, cs4, cs5, cs6, cs7, cs8 = st.columns([1, 1.6, 1.0, 1.0, 1.2, 1.0, 1.2, 1.2])
                cs1.write(f"**ID:** {eid}")
